...
// Unique context type to identify a context as being an RTU transport layer.
#define TBX_MB_RTU_CONTEXT_TYPE       ( 84U )
#define TBX_MB_RTU_STATE_INIT         ( 0U )     // Initial state.
#define TBX_MB_RTU_STATE_IDLE         ( 1U )     // Idle state. Ready to receive or transmit.
#define TBX_MB_RTU_STATE_TRANSMISSION ( 2U )     // Transmitting a PDU state.
#define TBX_MB_RTU_STATE_RECEPTION    ( 3U )     // Receiving a PDU state.
#define TBX_MB_RTU_STATE_VALIDATION   ( 4U )     // Validating a newly received PDU state.
...
/** \brief  RTU transport layer handle lookup table by UART port. Uses for finding
 *          the transport layer handle that uses a specific serial port, in a
 *          run-time efficient way.
 */
static volatile tTbxMbTpRtuCtx *tbxMbTpRtuCtx[ TBX_MB_UART_NUM_PORT ] = { 0 };

/**
 * \brief   Creates a Modbus RTU transport layer object.
 * \param   nodeAddr  The address of the node. Can be in the range 1..247 for
 *                    a server node. Set it to 0 for the client.
 * \param   port      The serial port to use. The actual meaning of the serial port
 *                    is hardware dependent. It typically maps to the UART peripheral
 *                    number. E.g. TBX_MB_UART_PORT1 = USART1 on an STM32.
 ...
 *          otherwise.
 */
tTbxMbTp TbxMbRtuCreate( uint8_t nodeAddr, tTbxMbUartPort port, tTbxMbUartBaudrate baudrate,
  ...
  TbxMbOsalEventInit( );
  /* Verify parameters. */
  TBX_ASSERT( ( nodeAddr <= TBX_MB_TP_NODE_ADDR_MAX ) && ( port < TBX_MB_UART_NUM_PORT ) &&
              ( baudrate < TBX_MB_UART_NUM_BAUDRATE ) && ( stopbits < TBX_MB_UART_NUM_STOPBITS ) &&
              ( parity < TBX_MB_UART_NUM_PARITY ) );
  /* Only continue with valid parameters. */
  if ( ( nodeAddr <= TBX_MB_TP_NODE_ADDR_MAX ) && ( port < TBX_MB_UART_NUM_PORT ) &&
       ( baudrate < TBX_MB_UART_NUM_BAUDRATE ) && ( stopbits < TBX_MB_UART_NUM_STOPBITS ) &&
       ( parity < TBX_MB_UART_NUM_PARITY ) ) {
...
static uint8_t TbxMbRtuTransmit( tTbxMbTp transport ) {
  ...
  if ( transport != NULL ) {
    ...
    if ( tpCtx->type == TBX_MB_RTU_CONTEXT_TYPE ) {
      ...
      TbxCriticalSectionEnter( );
      if ( ( tpCtx->txPacket.pdu.code & TBX_MB_FC_EXCEPTION_MASK ) == TBX_MB_FC_EXCEPTION_MASK ) {
...
static void TbxMbRtuDataReceived( tTbxMbUartPort port, uint8_t const *data, uint8_t len ) {
  /* Verify parameters. */
  TBX_ASSERT( ( port < TBX_MB_UART_NUM_PORT ) && ( data != NULL ) && ( len > 0U ) );

  /* Only continue with valid parameters. */
  if ( ( port < TBX_MB_UART_NUM_PORT ) && ( data != NULL ) && ( len > 0U ) ) {
    /* Obtain transport layer context linked to UART port of this event. */
    tTbxMbTpRtuCtx volatile *tpCtx = tbxMbTpRtuCtx[ port ];
    /* Verify transport layer context. */
    TBX_ASSERT( tpCtx != NULL )
 ...
 */
      ...
      /* Are we in the IDLE state? */
         ...
         */
        for ( uint8_t idx = 0U; idx < len; idx++ ) { aduPtr[ idx ] = data[ idx ]; }
      ...
      else { /* Nothing left to do, but MISRA requires this terminating else statement. */
      }
    }
  }
