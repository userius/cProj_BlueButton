...
void TbxMbOsalEventInit( void ) {
  ...
  if ( osalInitialized == TBX_FALSE ) {     // Only run this function once,
    osalInitialized = TBX_TRUE;
    TbxMbOsalEventQueueClear( );     // Clear the queue.
  }
  return;
}
 ...
 * \param     fromIsr   TBX_TRUE when calling this function from an interrupt
 *                      service routine, TBX_FALSE otherwise.
 */
void TbxMbOsalEventPost( tTbxMbEvent const *event, uint8_t fromIsr ) {
  //
  TBX_UNUSED_ARG( fromIsr );
  TBX_ASSERT( event != NULL );     // Verify parameters.
  if ( event != NULL ) {
 ...
 * \param     timeoutMs   Maximum time in milliseconds to block while waiting for an event.
 * \return    TBX_TRUE if an event occurred, TBX_FALSE otherwise (typically a timeout).
 */
uint8_t TbxMbOsalEventWait( tTbxMbEvent *event, uint16_t timeoutMs ) {
  //
  uint8_t result = TBX_FALSE;
  TBX_UNUSED_ARG( timeoutMs );
  TBX_ASSERT( event != NULL );     // Verify parameters.
  if ( event != NULL ) {           // Only continue with valid parameters.
 ...
 * \param     context   The context of the channel or transport layer, whose entries
 *                      need to be removed from the event queue.
 */
void TbxMbOsalEventPurge( void const *context ) {
  //
  TBX_ASSERT( context != NULL );     // Verify parameters.
  if ( context != NULL ) {           // Only continue with valid parameters.
    ...
    for ( size_t idx = 0U; idx < numEntries; idx++ ) {
      ...
      uint8_t     queueResult = TbxMbOsalEventQueueRead( &currentEvent );
...
/**
 * \brief     Creates a new binary semaphore object with an initial count of 0,
 *            meaning that it's taken.
 * \return    Handle to the newly created binary semaphore object if successful,
 *            NULL otherwise.
 */
...
/**
 * \brief     Releases a binary semaphore object,
 *            previously created with TbxMbOsalSemCreate().
 * \param     sem   Handle to the binary semaphore object to release.
 */
void TbxMbOsalSemFree( tTbxMbOsalSem sem ) {
  //
  TBX_ASSERT( sem != NULL );     // Verify parameters.
 ...
 *            timeout).
 */
uint8_t TbxMbOsalSemTake( tTbxMbOsalSem sem, uint16_t timeoutMs ) {
  //
  uint8_t result = TBX_FALSE;
  TBX_ASSERT( sem != NULL );     // Verify parameters.
  /* Only continue with valid parameters. */
       ...
       * before the specified timeout.        */
        ...
        while ( waitTimeMs > 0U ) {
          ...
          if ( deltaMs > 0U ) {
            ...
            if ( waitTimeMs >= deltaMs ) { waitTimeMs -= deltaMs; }
            else { waitTimeMs = 0U; }
          }
...
static void TbxMbOsalEventQueueClear( void ) {
  //
  TbxCriticalSectionEnter( );     // Clear the queue.
  eventQueue.count    = 0U;
...
static uint8_t TbxMbOsalEventQueueWrite( tTbxMbEvent const *event ) {
  ...
  if ( event != NULL ) {           // Only continue with valid parameters.
    ...
    if ( eventQueue.count < TBX_MB_EVENT_QUEUE_SIZE ) {
      ...
      if ( eventQueue.writeIdx == TBX_MB_EVENT_QUEUE_SIZE ) { eventQueue.writeIdx = 0U; }
      /* Update the result. */
      result = TBX_TRUE;
    }
    TbxCriticalSectionExit( );
 ...
 * \return    TBX_TRUE is successful, TBX_FALSE otherwise in case or
 *            error or if no event is currently present in the queue.
 */
static uint8_t TbxMbOsalEventQueueRead( tTbxMbEvent *event ) {
  //
  uint8_t result = TBX_FALSE;
  ...
  if ( event != NULL ) {           // Only continue with valid parameters.
    ...
    if ( eventQueue.count > 0U ) {
      ...
      *event = eventQueue.entries[ eventQueue.readIdx ];
      ...
      eventQueue.readIdx++;
      /* Time to wrap around to the start? */
      if ( eventQueue.readIdx == TBX_MB_EVENT_QUEUE_SIZE ) { eventQueue.readIdx = 0U; }
      /* Update the result. */
      result = TBX_TRUE;
    }
    TbxCriticalSectionExit( );
