...
/**
 * Sets many bits from a table of bytes (only the bits between
...
uint8_t modbus_get_byte_from_bits( const uint8_t *src, int idx, unsigned int nb_bits ) {
  ...
  for ( i = 0; i < nb_bits; i++ ) { value |= ( src[ idx + i ] << i ); }
...
float modbus_get_float( const uint16_t *src ) { return modbus_get_float_cdab( src ); }
...
void modbus_set_float( float f, uint16_t *dest ) { modbus_set_float_cdab( f, dest ); }
