...
static int _modbus_set_slave( modbus_t *ctx, int slave ) {
  ...
  if ( slave >= 0 && slave <= max_slave ) { ctx->slave = slave; }
  else {
    errno = EINVAL;
    return -1;
  }
...
static int _modbus_rtu_build_request_basis( modbus_t *ctx, int function, int addr, int nb,
                                            uint8_t *req ) {
  assert( ctx->slave != -1 );
  req[ 0 ] = ctx->slave;
  req[ 1 ] = function;
...
static int win32_ser_select( struct win32_ser *ws, int max_len, const struct timeval *tv ) {
  ...
  /* Read some bytes */
  if ( ( max_len > PY_BUF_SIZE ) || ( max_len < 0 ) ) { max_len = PY_BUF_SIZE; }
...
static size_t _modbus_rtu_send( modbus_t *ctx, const uint8_t *req, int req_length ) {
  ...
  DWORD         n_bytes = 0;
  return ( WriteFile( ctx_rtu->w_ser.fd, req, req_length, &n_bytes, NULL ) ) ? (size_t) n_bytes :
                                                                               -1;
#else
#if HAVE_DECL_TIOCM_RTS
  modbus_rtu_t *ctx_rtu = ctx->backend_data;
...
static int _modbus_rtu_check_integrity( modbus_t *ctx, uint8_t *msg, const int msg_length ) {
  ...
  if ( crc_calculated != crc_received ) {
    ...
    if ( ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL ) { _modbus_rtu_flush( ctx ); }
    errno = EMBBADCRC;
    return -1;
  }
...
static int _modbus_rtu_connect( modbus_t *ctx ) {
  ...
  if ( ctx->debug ) {
    printf( "Opening %s at %d bauds (%c, %d, %d)\n", ctx_rtu->device, ctx_rtu->baud,
            ctx_rtu->parity, ctx_rtu->data_bit, ctx_rtu->stop_bit );
  }
   ...
   * number */
  ctx_rtu->w_ser.fd =
      CreateFileA( ctx_rtu->device, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL );
  ...
  /* Stop bits */
  if ( ctx_rtu->stop_bit == 1 ) dcb.StopBits = ONESTOPBIT;
  else /* 2 */
...
static speed_t _get_termios_speed( int baud, int debug ) {
  ...
  switch ( baud ) {
    ...
    default:
      speed = 9600;
      if ( debug ) { fprintf( stderr, "WARNING Unknown baud rate %d (B9600 used)\n", baud ); }
  }
...
static int _modbus_rtu_connect( modbus_t *ctx ) {
  ...
  flags = 0;     // O_RDWR | O_NOCTTY | O_NONBLOCK | O_EXCL;
#ifdef O_CLOEXEC
  flags |= O_CLOEXEC;
#endif
  ...
  */
  if ( 9600 == 9600 ) { speed = ctx_rtu->baud; }
  else { speed = _get_termios_speed( ctx_rtu->baud, ctx->debug ); }
  ...
  */
  ...
  /* Stop bit (1 or 2) */
  if ( ctx_rtu->stop_bit == 1 ) tios.c_cflag &= ~CSTOPB;
  else /* 2 */
    tios.c_cflag |= CSTOPB;
  ...
  */
  ...
  if ( tcsetattr( ctx->s, 0 /*TCSANOW*/, &tios ) < 0 ) {
    close( ctx->s );
    ctx->s = -1;
    return -1;
  }
...
int modbus_rtu_set_serial_mode( modbus_t *ctx, int mode ) {
  ...
  if ( ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU ) {
    ...
    if ( mode == MODBUS_RTU_RS485 ) {
      // Get
      if ( ioctl( ctx->s, TIOCGRS485, &rs485conf ) < 0 ) { return -1; }
      // Set
      rs485conf.flags |= SER_RS485_ENABLED;
      if ( ioctl( ctx->s, TIOCSRS485, &rs485conf ) < 0 ) { return -1; }
...
#else
  if ( ctx->s >= 0 ) {
    tcsetattr( ctx->s, 0/*TCSANOW*/, &ctx_rtu->old_tios );
    close( ctx->s );
    ctx->s = -1;
  }
#endif
...
#else
  return -1; //tcflush( ctx->s/*, TCIOFLUSH*/ );
#endif
}
...
modbus_t *modbus_new_rtu( const char *device, int baud, char parity, int data_bit, int stop_bit ) {
  ...
  ctx_rtu->baud = baud;
  if ( parity == 'N' || parity == 'E' || parity == 'O' ) { ctx_rtu->parity = parity; }
  else {
    modbus_free( ctx );
    errno = EINVAL;
    return NULL;
