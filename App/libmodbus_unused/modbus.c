...
/* Exported version */
const unsigned int libmodbus_version_major = 3;      // LIBMODBUS_VERSION_MAJOR;
const unsigned int libmodbus_version_minor = 1;      // LIBMODBUS_VERSION_MINOR;
const unsigned int libmodbus_version_micro = 11;     // LIBMODBUS_VERSION_MICRO;
...
static unsigned int compute_response_length_from_request( modbus_t *ctx, uint8_t *req ) {
  ...
  switch ( req[ offset ] ) {
    ...
    case MODBUS_FC_READ_INPUT_REGISTERS:
      ...
      break;
    case MODBUS_FC_READ_EXCEPTION_STATUS: length = 3; break;
    case MODBUS_FC_REPORT_SLAVE_ID:
      /* The response is device specific (the header provides the
         length) */
      return MSG_LENGTH_UNDEFINED;
    case MODBUS_FC_MASK_WRITE_REGISTER: length = 7; break;
    default: length = 5;
  }
...
int modbus_send_raw_request_tid( modbus_t *ctx, const uint8_t *raw_req, int raw_req_length,
                                 int tid ) {
  sft_t   sft;
...
static uint8_t compute_meta_length_after_function( int function, msg_type_t msg_type ) {
  ...
  if ( msg_type == MSG_INDICATION ) {
    if ( function <= MODBUS_FC_WRITE_SINGLE_REGISTER ) { length = 4; }
    else if ( function == MODBUS_FC_WRITE_MULTIPLE_COILS ||
              function == MODBUS_FC_WRITE_MULTIPLE_REGISTERS ) {
      length = 5;
    }
    else if ( function == MODBUS_FC_MASK_WRITE_REGISTER ) { length = 6; }
    else if ( function == MODBUS_FC_WRITE_AND_READ_REGISTERS ) { length = 9; }
    else {
    ...
    switch ( function ) {
      ...
      case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: length = 4; break;
      case MODBUS_FC_MASK_WRITE_REGISTER: length = 6; break;
      default: length = 1;
    }
  }
...
int _modbus_receive_msg( modbus_t *ctx, uint8_t *msg, msg_type_t msg_type ) {
  ...
  if ( ctx->debug ) {
    if ( msg_type == MSG_INDICATION ) { printf( "Waiting for an indication...\n" ); }
    else { printf( "Waiting for a confirmation...\n" ); }
  }

  if ( !ctx->backend->is_connected( ctx ) ) {
    if ( ctx->debug ) { fprintf( stderr, "ERROR The connection is not established.\n" ); }
    return -1;
  }
...
static int check_confirmation( modbus_t *ctx, uint8_t *req, uint8_t *rsp, int rsp_length ) {
  ...
  if ( function >= 0x80 ) {
    if ( rsp_length == (int) ( offset + 2 + ctx->backend->checksum_length ) &&
      ...
      if ( exception_code < MODBUS_EXCEPTION_MAX ) { errno = MODBUS_ENOBASE + exception_code; }
      else { errno = EMBBADEXC; }
      _error_print( ctx, NULL );
      return -1;
    }
    else {
  ...
  if ( ( rsp_length == rsp_length_computed || rsp_length_computed == MSG_LENGTH_UNDEFINED ) &&
    ...
    if ( ( req_nb_value == rsp_nb_value ) && ( resp_addr_ok == TRUE ) &&
      ...
      if ( ctx->debug ) {
        fprintf( stderr, "Received data not corresponding to the request (%d != %d)\n",
...
static int response_io_status( uint8_t *tab_io_status, int address, int nb, uint8_t *rsp,
                               int offset ) {
  int shift = 0;
...
static int response_exception( modbus_t *ctx, sft_t *sft, int exception_code, uint8_t *rsp,
                               unsigned int to_flush, const char *template, ... ) {
  int rsp_length;
...
int modbus_reply( modbus_t *ctx, const uint8_t *req, int req_length,
  ...
  switch ( function ) {
    ...
    case MODBUS_FC_READ_DISCRETE_INPUTS: {
      ...
      if ( nb < 1 || MODBUS_MAX_READ_BITS < nb ) {
        ...
        rsp_length = response_exception( ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp,
                                         FALSE, "Illegal data address 0x%0X in %s\n",
    ...
    case MODBUS_FC_WRITE_SINGLE_COIL: {
      ...
      if ( mapping_address < 0 || mapping_address >= mb_mapping->nb_bits ) {
        rsp_length =
            response_exception( ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp, FALSE,
      ...
      if ( rsp_length != req_length ) {
        ...
        rsp_length = response_exception(
            ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, FALSE,
    ...
    case MODBUS_FC_WRITE_MULTIPLE_COILS: {
      ...
      if ( nb < 1 || MODBUS_MAX_WRITE_BITS < nb || nb_bits * 8 < nb ) {
        ...
        rsp_length = response_exception( ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp,
                                         FALSE, "Illegal data address 0x%0X in write_bits\n",
    ...
    case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
      ...
      if ( nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb || nb_bytes != nb * 2 ) {
        rsp_length =
            response_exception( ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, TRUE,
                                "Illegal number of values %d in write_registers (max %d)\n", nb,
                                MODBUS_MAX_WRITE_REGISTERS );
      }
      else if ( mapping_address < 0 || ( mapping_address + nb ) > mb_mapping->nb_registers ) {
        rsp_length = response_exception( ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp,
                                         FALSE, "Illegal data address 0x%0X in write_registers\n",
                                         mapping_address < 0 ? address : address + nb );
      }
      else {
        int i, j;
    ...
    case MODBUS_FC_MASK_WRITE_REGISTER: {
      ...
      if ( mapping_address < 0 || mapping_address >= mb_mapping->nb_registers ) {
        ...
        if ( rsp_length != req_length ) {
          /* Bad use of modbus_reply */
          rsp_length =
              response_exception( ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, FALSE,
  ...
  if ( ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU &&
       slave == MODBUS_BROADCAST_ADDRESS && !( ctx->quirks & MODBUS_QUIRK_REPLY_TO_BROADCAST ) ) {
    return 0;
  }
...
int modbus_write_bits( modbus_t *ctx, int addr, int nb, const uint8_t *src ) {
  ...
  req_length =
      ctx->backend->build_request_basis( ctx, MODBUS_FC_WRITE_MULTIPLE_COILS, addr, nb, req );
...
int modbus_write_registers( modbus_t *ctx, int addr, int nb, const uint16_t *src ) {
  ...
  req_length =
      ctx->backend->build_request_basis( ctx, MODBUS_FC_WRITE_MULTIPLE_REGISTERS, addr, nb, req );
  byte_count          = nb * 2;
  req[ req_length++ ] = byte_count;
...

{
  ...
  req_length = ctx->backend->build_request_basis( ctx, MODBUS_FC_WRITE_AND_READ_REGISTERS,
...
int modbus_report_slave_id( modbus_t *ctx, int max_dest, uint8_t *dest ) {
  ...
  if ( rc > 0 ) {
    ...
    for ( i = 0; i < rc && i < max_dest; i++ ) { dest[ i ] = rsp[ offset + i ]; }
  }
...
modbus_mapping_t *modbus_mapping_new_start_address(
  ...
  if ( mb_mapping == NULL ) { return NULL; }

  /* 0X */
  mb_mapping->nb_bits    = nb_bits;
  mb_mapping->start_bits = start_bits;
  if ( nb_bits == 0 ) { mb_mapping->tab_bits = NULL; }
  else {
  ...
  if ( nb_input_bits == 0 ) { mb_mapping->tab_input_bits = NULL; }
  else {
  ...
  if ( nb_registers == 0 ) { mb_mapping->tab_registers = NULL; }
  else {
  ...
  if ( nb_input_registers == 0 ) { mb_mapping->tab_input_registers = NULL; }
  else {
...
modbus_mapping_t *modbus_mapping_new( int nb_bits, int nb_input_bits, int nb_registers,
  ...
  return modbus_mapping_new_start_address( 0, nb_bits, 0, nb_input_bits, 0, nb_registers, 0,
                                           nb_input_registers );
}
